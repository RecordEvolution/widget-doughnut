...
- The host platform passes data via two reactive properties: `inputData: InputData` (data + per-series settings) and `theme: { theme_name, theme_object }` (an ECharts theme). Theme can also be supplied via CSS custom properties `--re-text-color` and `--re-tile-background-color`.
